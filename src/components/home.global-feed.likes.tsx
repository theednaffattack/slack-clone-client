import { Alert, AlertIcon, AlertTitle, Button, CloseButton, Flex, Text } from "@chakra-ui/core";
import React, { useState } from "react";
import { AiFillHeart, AiOutlineHeart } from "react-icons/ai";
import { FaRegComment } from "react-icons/fa";
import {   PostConnection, useCreateOrUpdateLikesMutation } from "../generated/graphql";
...
export function LikesAndCommentsSummary({
  ...
  const [errorFlashes, setErrorFlashes] = useState<"hidden" | "visible">("hidden")
  const [
    createOrUpdateLikes
...
<Flex>
  { errorFlashes === "visible" ? <ErrorFlash errorMessage="Login to vote." setErrorFlashes={setErrorFlashes} /> : ""}
      </Flex>
      ...
      <Flex>
        ...
        fields: {
          getGlobalPostsRelay(existingPosts): PostConnection {
            const { edges, __typename, pageInfo } = existingPosts as any;

            const postToUpdate = edges.filter(({node}: any)=>node.__ref === `GlobalPostReturnType:${likeData?.createOrUpdateLikes?.postId}`)
            return {
              edges: [
                {
                  __typename: "PostEdge",
                  ...
                  node: {
                    ...
                    likes_count: postToUpdate[0].node.likes_count - 1,
              ...
              pageInfo
                ...
                console.log("UPDATE LIKES ERROR - CURRENTLY LIKED", error.message);
                    ...
                    cache.modify({
                      fields: {
                        getGlobalPostsRelay(existingPosts): PostConnection {
                          ...
                          const postToUpdate = edges.filter(({node}: any)=>{

                            
                            return node.__ref === `GlobalPostReturnType:${likeData?.createOrUpdateLikes?.postId}`})
                          return {
                            edges: [
                              {
                                __typename: "PostEdge",
  ...
  console.log("UPDATE LIKES ERROR - NOT CURRENTLY LIKED", error.message);
...
<Alert
  ...
  <CloseButton position="absolute" right="8px" top="8px" disabled={!errorMessage} onClick={()=>setErrorFlashes("hidden")} />
</Alert>
  )
}